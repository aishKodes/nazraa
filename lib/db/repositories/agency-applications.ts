...
export async function listAgencyApplications(scope: Scope): Promise<AgencyReviewApplication[]> {
  ...
  try {
    ...
    let creations: (RowDataPacket & { id: string; full_name: string; public_id: number; agency_name: string; country_code: string; business_whatsapp_e164: string; status: string; created_at: string })[] = [];
    if (scope.account.role === "MASTER" || scope.account.role === "ADMIN") {
      const [rows] = await db().query<typeof creations>(
        `SELECT application.id, user.full_name, user.public_id, application.agency_name,
                application.country_code, application.business_whatsapp_e164, application.status, application.created_at
    ...
    return [
      ...joins.map((row) => ({ id: row.id, type: "JOIN" as const, userName: row.full_name, userPublicId: String(row.public_id), agencyName: row.agency_name, agencyPublicId: String(row.agency_public_id), countryCode: null, whatsapp: null, status: row.status, createdAt: row.created_at })),
      ...creations.map((row) => ({ id: row.id, type: "CREATE" as const, userName: row.full_name, userPublicId: String(row.public_id), agencyName: row.agency_name, agencyPublicId: null, countryCode: row.country_code, whatsapp: row.business_whatsapp_e164, status: row.status, createdAt: row.created_at })),
...
export async function reviewAgencyCreation(input: { scope: Scope; applicationId: string; decision: "APPROVED" | "REJECTED"; reason: string; parentAccountId?: string }) {
  ...
  await withTransaction(async (connection) => {
    const [rows] = await connection.query<(RowDataPacket & { id: string; application_user_id: string; agency_name: string; country_code: string; business_whatsapp_e164: string; status: string })[]>("SELECT id, application_user_id, agency_name, country_code, business_whatsapp_e164, status FROM agency_creation_applications WHERE id = ? LIMIT 1 FOR UPDATE", [input.applicationId]);
    const application = rows[0];
    ...
    if (input.decision === "APPROVED") {
      const parentId = input.scope.account.role === "ADMIN" ? input.scope.account.id : input.parentAccountId;
      if (!parentId) throw new Error("Choose the Admin that will manage this Agency.");
      ...
      if (!parents[0]) throw new Error("The selected active Admin could not be found.");
      const [users] = await connection.query<(RowDataPacket & { agency_account_id: string | null })[]>("SELECT agency_account_id FROM application_users WHERE id = ? LIMIT 1 FOR UPDATE", [application.application_user_id]);
