...
export async function getHostDetail(scope: Scope, hostId: string) {
  ...
  const [rows] = await db().query<(RowDataPacket & { id: string; application_user_id: string; legal_name: string | null; full_name: string; public_id: number; external_user_id: string; avatar_url: string | null; date_of_birth: string | null; gender: string | null; whatsapp_e164: string | null; email: string | null; account_status: string; face_verification_status: string; country_code: string | null; status: string; verification_status: string; government_id_type: string | null; government_id_last4: string | null; agency_account_id: string | null; agency_name: string | null; applied_at: string; reviewed_at: string | null; review_reason: string | null; live_minutes_30d: number; session_count_30d: number; current_eligible_seconds: number | null; current_publishing_seconds: number | null; current_reconnect_state: string | null; current_accounting_accuracy: string | null; current_reward_units: number | null })[]>(
    `SELECT h.id, h.application_user_id, h.legal_name, u.full_name, u.public_id, u.external_user_id,
            ...
            ,COALESCE((SELECT SUM(accounting.eligible_seconds_committed)
              ...
              WHERE accounting.host_application_user_id = h.application_user_id
                ...
                AND accounting.started_at >= CURRENT_TIMESTAMP(3) - INTERVAL 30 DAY) DIV 60, 0) live_minutes_30d,
            (SELECT COUNT(*) FROM live_session_accounting accounting
              WHERE accounting.host_application_user_id = h.application_user_id
                AND accounting.room_type IN ('LIVE', 'FACE')
                AND accounting.started_at >= CURRENT_TIMESTAMP(3) - INTERVAL 30 DAY) session_count_30d,
            (SELECT accounting.eligible_duration_seconds + accounting.media_segment_seconds
              FROM live_session_accounting accounting
              WHERE accounting.host_application_user_id = h.application_user_id AND accounting.status = 'ACTIVE'
              ORDER BY accounting.started_at DESC LIMIT 1) current_eligible_seconds,
            (SELECT accounting.eligible_seconds_committed
              FROM live_session_accounting accounting
              WHERE accounting.host_application_user_id = h.application_user_id AND accounting.status = 'ACTIVE'
              ORDER BY accounting.started_at DESC LIMIT 1) current_publishing_seconds,
            (SELECT accounting.reconnect_state FROM live_session_accounting accounting
              WHERE accounting.host_application_user_id = h.application_user_id AND accounting.status = 'ACTIVE'
              ORDER BY accounting.started_at DESC LIMIT 1) current_reconnect_state,
            (SELECT accounting.accounting_accuracy FROM live_session_accounting accounting
              WHERE accounting.host_application_user_id = h.application_user_id AND accounting.status = 'ACTIVE'
              ORDER BY accounting.started_at DESC LIMIT 1) current_accounting_accuracy,
            (SELECT accounting.reward_units_generated FROM live_session_accounting accounting
              WHERE accounting.host_application_user_id = h.application_user_id AND accounting.status = 'ACTIVE'
              ORDER BY accounting.started_at DESC LIMIT 1) current_reward_units
  ...
  return rows[0] ? { id: rows[0].id, applicationUserId: rows[0].application_user_id, publicId: String(rows[0].public_id), legalName: rows[0].legal_name ?? rows[0].full_name, displayName: rows[0].full_name, externalUserId: rows[0].external_user_id, avatarUrl: rows[0].avatar_url, dateOfBirth: rows[0].date_of_birth, gender: rows[0].gender, mobile: rows[0].whatsapp_e164, email: rows[0].email, accountStatus: rows[0].account_status, faceVerification: rows[0].face_verification_status, country: rows[0].country_code, status: rows[0].status, verification: rows[0].verification_status, governmentIdType: rows[0].government_id_type, governmentIdLast4: rows[0].government_id_last4, agencyId: rows[0].agency_account_id, agencyName: rows[0].agency_name, appliedAt: rows[0].applied_at, reviewedAt: rows[0].reviewed_at, reviewReason: rows[0].review_reason, liveMinutes30d: Number(rows[0].live_minutes_30d), sessionCount30d: Number(rows[0].session_count_30d), currentLive: rows[0].current_reconnect_state == null ? null : { eligibleSeconds: Number(rows[0].current_eligible_seconds ?? 0), publishingSeconds: Number(rows[0].current_publishing_seconds ?? 0), reconnectState: String(rows[0].current_reconnect_state), accountingAccuracy: String(rows[0].current_accounting_accuracy ?? 'CONFIRMED'), rewardUnitsGenerated: Number(rows[0].current_reward_units ?? 0) } } : null;
}
