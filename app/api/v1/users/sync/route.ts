...
export async function POST(request: Request) {
  ...
  const parsed = z.object({ externalUserId: z.string().trim().min(1).max(80), fullName: z.string().trim().min(2).max(120), countryCode: z.string().trim().length(2).transform((value) => value.toUpperCase()), avatarUrl: z.string().url().optional(), agencyCode: z.string().trim().max(32).optional() }).safeParse(await request.json());
