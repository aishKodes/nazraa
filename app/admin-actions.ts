...
export async function submitRoomFeatureSettings(formData: FormData) {
  ...
  const parsed = z.object({
    ...
    faceViewerTransport: z.enum(["hls", "l3", "auto"]),
    faceL3Enabled: z.enum(["true", "false"]),
    faceL3ReviewerOnly: z.enum(["true", "false"]),
    faceSingleLayer720Enabled: z.enum(["true", "false"]),
    nazraaNaturalBeautyEnabled: z.enum(["true", "false"]),
    nazraaNaturalBeautyReviewerQaEnabled: z.enum(["true", "false"]),
    nazraaNaturalBeautyLandmarksEnabled: z.enum(["true", "false"]),
    nazraaNaturalBeautyDefaultStrength: z.coerce.number().int().min(0).max(100),
    passiveEventDelaySeconds: z.coerce.number().int().min(0).max(15),
    partyStreamingThreshold: z.coerce.number().int().min(1).max(200),
    passiveRtcAllowed: z.enum(["true", "false"]),
    maxGlobalRtcParticipants: z.coerce.number().int().min(1).max(500),
    mediaPublishingEnabled: z.enum(["true", "false"]),
  ...
  await saveRoomFeatureSettings({
    ...
    faceViewerTransport: parsed.data.faceViewerTransport,
    faceL3Enabled: parsed.data.faceL3Enabled === "true",
    faceL3ReviewerOnly: parsed.data.faceL3ReviewerOnly === "true",
    faceSingleLayer720Enabled: parsed.data.faceSingleLayer720Enabled === "true",
    nazraaNaturalBeautyEnabled: parsed.data.nazraaNaturalBeautyEnabled === "true",
    nazraaNaturalBeautyReviewerQaEnabled: parsed.data.nazraaNaturalBeautyReviewerQaEnabled === "true",
    nazraaNaturalBeautyLandmarksEnabled: parsed.data.nazraaNaturalBeautyLandmarksEnabled === "true",
    nazraaNaturalBeautyDefaultStrength: parsed.data.nazraaNaturalBeautyDefaultStrength,
    passiveEventDelaySeconds: parsed.data.passiveEventDelaySeconds,
    partyStreamingThreshold: parsed.data.partyStreamingThreshold,
    passiveRtcAllowed: parsed.data.passiveRtcAllowed === "true",
    maxGlobalRtcParticipants: parsed.data.maxGlobalRtcParticipants,
    mediaPublishingEnabled: parsed.data.mediaPublishingEnabled === "true",
