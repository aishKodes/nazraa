...
export default async function FaceVerificationPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; page?: string; view?: string }> }) {
  const scope = await requirePermission("face_verification.read");
  const { error, success, page: rawPage, view: rawView } = await searchParams;
  const page = Math.max(1, Number.parseInt(rawPage ?? "1", 10) || 1);
  const view = rawView === "verified" ? "verified" : "review";
  const rows = await listFaceVerificationRequests(scope, page, view);
  ...
  return <>
    ...
    <div className="filter-bar" aria-label="Verification lists">
      <Link className={view === "review" ? "primary-button" : "secondary-button"} href="/dashboard/face-verification">Pending / needs attention</Link>
      <Link className={view === "verified" ? "primary-button" : "secondary-button"} href="/dashboard/face-verification?view=verified">Verified</Link>
    </div>
    ...
    </tr>)}</tbody></table></div> : <EmptyState title={view === "verified" ? "No verified users" : "No pending verification"} detail={view === "verified" ? "Approved users appear in this separate list." : "New submissions and users needing attention appear here."} />}<Pagination path="/dashboard/face-verification" page={page} hasNext={rows.length > 25} query={{ view: view === "verified" ? "verified" : undefined }} /></Card>
