...
export default async function FaceVerificationPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; page?: string }> }) {
  ...
  return <>
    ...
    <Card>{requests.length ? <div className="table-scroll"><table><thead><tr>
      ...
      {manageLegacy ? <td data-label="Review">{request.status !== "VERIFIED" ? <form action={submitFaceVerificationReview} className="inline-review"><input type="hidden" name="requestId" value={request.id} /><select name="decision" defaultValue="" required><option value="" disabled>Decision…</option><option value="VERIFIED">Approve</option><option value="REJECTED">Reject</option></select><input name="reason" minLength={5} maxLength={500} required placeholder="Review reason" /><button className="table-button" type="submit">Save</button></form> : <span className="verification-approved">Approved</span>}</td> : null}
