  ...
  return <>
    ...
    <Card className="settings-card">
      ...
      <div className="stack-form full-width">
        {configurableGameIds.map((gameId) => {
          ...
          const name = ({ teen_patti_pro: "Teen Patti Pro", luck77: "77 – LUCKY SEVEN", greedy_lion: "Greedy Lion", greedy_king: "Greedy King", bounty_football: "Bounty Football", jungle_hunt: "Jungle Hunt" } as const)[gameId];
    ...
    <Card className="settings-card">
      ...
      <form action={submitRoomFeatureSettings} className="form-grid">
        ...
        <label>Party CDN listener threshold<input name="partyStreamingThreshold" type="number" min="1" max="200" required defaultValue={roomFeatures?.partyStreamingThreshold ?? 1} /><span>Normal production starts CDN for the first passive listener. This value is used only if the audited passive-RTC break-glass switch is enabled.</span></label>
        <label>Allow passive RTC<select name="passiveRtcAllowed" defaultValue={String(roomFeatures?.passiveRtcAllowed === true)}><option value="false">Off — required production protection</option><option value="true">Break-glass only</option></select><span>Muted/view-only RTC users still consume billable participant minutes. Keep off unless a time-limited incident response explicitly requires it.</span></label>
        <label>Maximum global RTC participants<input name="maxGlobalRtcParticipants" type="number" min="1" max="500" required defaultValue={roomFeatures?.maxGlobalRtcParticipants ?? 24} /><span>Hard account-wide circuit breaker for Hosts and accepted speakers only. Passive CDN audience is never counted or granted RTC.</span></label>
        <label>Media publishing emergency switch<select name="mediaPublishingEnabled" defaultValue={String(roomFeatures?.mediaPublishingEnabled !== false)}><option value="true">Live publishing enabled</option><option value="false">Stop new RTC publishers</option></select><span>Emergency only. It blocks new Host/speaker RTC credentials but does not alter wallet, moderation, or existing recorded data.</span></label>
        <label>Face CDN warm output<select name="faceCdnKeepWarmWhileHostLive" defaultValue={String(roomFeatures?.faceCdnKeepWarmWhileHostLive === true)}><option value="false">Start when first viewer joins — production</option><option value="true">Keep warm while Host is live</option></select><span>Warm outputs consume Live Creation minutes even with no passive audience. Keep the production default off; the first viewer receives a bounded stream-start state rather than RTC fallback.</span></label>
