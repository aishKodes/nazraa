...
export default async function RoomsPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; page?: string }> }) {
  ...
  return <>
    ...
    {mayViewCostTelemetry ? <Card>
      <div className="card-title"><div><h2>Mobile latency diagnostics</h2><p>Server-side rolling 7-day samples (5% for room presence/PK, 10% for other mobile requests). Sample counts are not total traffic. Network, Flutter route, and first-frame time are measured separately by the client; these values isolate API and MySQL work.</p></div></div>
