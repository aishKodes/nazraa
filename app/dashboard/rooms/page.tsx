...
export default async function RoomsPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; page?: string }> }) {
  ...
  return <>
    ...
    <Card>{incidents.length ? <div className="table-scroll"><table><thead><tr><th>Host</th><th>Room</th><th>Event</th><th>Failures</th><th>Time</th><th>Live access</th></tr></thead><tbody>{incidents.map((incident) => <tr key={incident.id}>
      ...
      <td data-label="Live access">{incident.restrictionId ? mayManage ? <details className="row-action"><summary>Restricted · review</summary><form action={submitRestoreLiveAccess}><input type="hidden" name="restrictionId" value={incident.restrictionId} /><input name="reason" minLength={5} maxLength={500} required placeholder="Restoration reason" /><button className="secondary-button" type="submit">Restore Live access</button></form></details> : <StatusBadge value="SUSPENDED" /> : <StatusBadge value="ACTIVE" />}</td>
