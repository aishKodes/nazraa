...
export default async function WalletPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const inventoryAccounts = accounts.filter((account) => ["MASTER", "ADMIN", "COIN_SELLER"].includes(account.role) && account.status === "ACTIVE");
  return <>
