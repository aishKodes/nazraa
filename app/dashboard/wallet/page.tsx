...
import { submitCoinInventoryAdjustment, submitCoinTransfer } from "@/app/actions";
...
export default async function WalletPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [users, ledger, accounts] = await Promise.all([listUsers(scope), getRecentLedger(scope), scope.account.role === "MASTER" ? listPlatformAccounts(scope) : Promise.resolve([])]);
  const allowed = can(scope.account.role, "coins.transfer");
  const idempotencyKey = randomUUID();
  const allocationKey = randomUUID();
  const inventoryAccounts = accounts.filter((account) => ["MASTER", "ADMIN", "AGENCY", "COIN_SELLER"].includes(account.role) && account.status === "ACTIVE");
  return <>
    ...
    {can(scope.account.role, "coins.allocate") ? <Card className="inventory-card"><div className="card-title"><div><h2>Allocate coin inventory</h2><p>Master-only control for funding or reducing an Admin, Agency, or Coin Seller wallet.</p></div></div><form action={submitCoinInventoryAdjustment} className="form-grid"><input type="hidden" name="idempotencyKey" value={allocationKey} /><label>Account<select name="accountId" required defaultValue=""><option value="" disabled>Select account</option>{inventoryAccounts.map((account) => <option value={account.id} key={account.id}>{account.role.replaceAll("_", " ")} · {account.name} · {account.code}</option>)}</select></label><label>Action<select name="direction" defaultValue="ADD"><option value="ADD">Add inventory</option><option value="REMOVE">Remove inventory</option></select></label><label>Coins<input name="amount" type="number" inputMode="numeric" min="1" required /></label><label className="span-two">Reason<input name="reason" minLength={5} maxLength={500} required placeholder="Why is this inventory change needed?" /></label><label>Confirm<button className="primary-button" type="submit">Update inventory</button></label></form></Card> : null}
