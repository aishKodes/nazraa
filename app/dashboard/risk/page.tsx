...
export default async function RiskPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  const scope = await requirePermission("risk.read");
  const { error, success } = await searchParams; const flags = await listRiskFlags(scope); const manage = can(scope.account.role, "risk.manage");
