...
export default async function SupportPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; page?: string }> }) {
  const scope = await requirePermission("support.read"); const { error, success, page: rawPage } = await searchParams; const page = Math.max(1, Number.parseInt(rawPage ?? "1", 10) || 1); const rows = await listSupportTickets(scope, page); const tickets = rows.slice(0, 25); const manage = can(scope.account.role, "support.manage");
