...
export default async function AgenciesPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string }> }) {
  ...
  const [{ error, success }, agencies, applications] = await Promise.all([
    searchParams,
    listAgencies(scope),
  ...
  return <>
    ...
    {applications.length ? <Card><h2>Mobile applications</h2><div className="table-scroll"><table><thead><tr><th>Applicant</th><th>Request</th><th>Agency / parent</th><th>Protected KYC</th><th>Submitted</th><th>Status</th><th>Review</th></tr></thead><tbody>{applications.map((application) => <tr key={application.id}><td><b>{application.ownerName ?? application.userName}</b><small className="block">App user: {application.userName}</small><small className="block mono">ID {application.userPublicId}</small></td><td>{application.type === "JOIN" ? "Join" : "Create"}{application.countryCode ? <small className="block">{application.countryCode} · {application.whatsapp}</small> : null}</td><td><b>{application.agencyName}</b>{application.agencyPublicId ? <small className="block mono">Agency ID {application.agencyPublicId}</small> : null}{application.parentPublicId ? <small className="block">Under {application.parentRole} {application.parentName} · <span className="mono">{application.parentPublicId}</span></small> : null}</td><td>{application.type === "CREATE" ? <><small className="block">PAN {application.panMasked ?? "legacy"}</small><small className="block">Aadhaar {application.aadhaarMasked ?? "legacy"}</small><div className="account-actions">{application.panMasked ? <a className="table-link" href={`/api/agency-applications/${application.id}/pan`}>View PAN</a> : null}{application.aadhaarMasked ? <a className="table-link" href={`/api/agency-applications/${application.id}/aadhaar`}>View Aadhaar</a> : null}{application.hasDocument ? <a className="table-link" href={`/api/agency-applications/${application.id}/document`}>Proof</a> : null}</div></> : "—"}</td><td>{formatDate(application.createdAt)}</td><td><StatusBadge value={application.status} /></td><td>{application.status === "PENDING" ? <form action={submitAgencyApplicationReview} className="inline-action-form"><input type="hidden" name="applicationId" value={application.id} /><input type="hidden" name="type" value={application.type} /><select name="decision" defaultValue="APPROVED"><option value="APPROVED">Approve</option><option value="REJECTED">Reject</option></select><input name="reason" required minLength={5} maxLength={500} placeholder="Review reason" /><button type="submit" className="table-link button-link">Save</button></form> : "—"}</td></tr>)}</tbody></table></div></Card> : <Card><EmptyState title="No Agency applications" detail="New mobile join and creation requests will appear here." /></Card>}
